...
//@input Component.ScriptComponent[] scriptsWithCallbacks {"hint": "If these scripts have function script.onFound or script.api.onLost they will be called correspondingly"}
...
function onUpdate() {
    ...
    if (script.showDebug && script.debugScoreBar && script.debugScoreBar.updateValue) {
        script.debugScoreBar.updateValue(currentProb);
    }
}
...
function invokeOnFoundCallbacks() {
    for (var i = 0; i < script.scriptsWithCallbacks.length; i++) {
        if (script.scriptsWithCallbacks[i] && script.scriptsWithCallbacks[i].onFound) {
            script.scriptsWithCallbacks[i].onFound();
        }
    }
}
...
function invokeOnLostCallbacks() {
    for (var i = 0; i < script.scriptsWithCallbacks.length; i++) {
        if (script.scriptsWithCallbacks[i] && script.scriptsWithCallbacks[i].onLost) {
            script.scriptsWithCallbacks[i].onLost();
        }
    }
}
...
function checkInputs() {
    ...
    if (script.showDebug) {
        if (!script.debugScoreBar) {
            debugPrint("Error, debugScoreBar is not set");
            return false;
        }
